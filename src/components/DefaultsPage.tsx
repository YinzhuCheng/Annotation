...
export function DefaultsPage(props: { onBack: () => void }) {
  ...
  const applyOptionsCountToExisting = useAppStore(
    (s) => s.applyOptionsCountToExisting,
  );
  ...
  useEffect(() => {
    ...
    setMaxReviewRounds(defaults.maxReviewRounds || 3);
  }, [defaults]);
  ...
  const handleAuth = (e: FormEvent) => {
    ...
    if (password === "111111") {
      ...
      try {
        sessionStorage.setItem("defaults-admin-authed", "1");
      } catch {}
    } else {
  ...
  const onConfirm = () => {
    const nextOptionsCount = Math.max(
      2,
      Math.min(
        10,
        Math.floor(Number(optionsCount) || defaults.optionsCount || 5),
      ),
    );
    const nextReviewRounds = Math.max(
      1,
      Math.min(
        10,
        Math.floor(Number(maxReviewRounds) || defaults.maxReviewRounds || 3),
      ),
    );
    const payload = {
      ...
      maxReviewRounds: nextReviewRounds,
    };
    setDefaults(payload);
      ...
      {!authorized ? (
        ...
        >
          {renderListEditor(
            ...
            (value) => removeItem(subfields, value, setSubfields),
            listHint,
          )}
          ...
          {renderListEditor(
            ...
            (value) => removeItem(sources, value, setSources),
            listHint,
          )}
          ...
          {renderListEditor(
            ...
            (value) => removeItem(academicLevels, value, setAcademicLevels),
            t("defaultsAcademicHint"),
          )}
          ...
          <div>
            {renderListEditor(
              ...
              (value) => removeItem(difficulties, value, setDifficulties),
              t("defaultsDifficultyHint"),
            )}
          ...
          <div>
            <div className="label">{t("defaultsMaxReviewRoundsLabel")}</div>
            <div className="small" style={{ marginBottom: 6 }}>
              {t("defaultsMaxReviewRoundsHint")}
            </div>
            <input
              type="number"
              min={1}
              ...
              onChange={(e) => setMaxReviewRounds(Number(e.target.value) || 0)}
            />
          </div>
        </div>
      )}
