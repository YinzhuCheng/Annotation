...
const resources = {
  en: {
    translation: {
      ...
      llmAssistGenerateHint: 'Let the generator rephrase the current draft to match the selected problem type, add any missing definitions, and autofill structured fields. Review the output after each run—your manual edits are preserved whenever possible.',
  ...
  zh: {
    translation: {
      ...
      llmAssist: 'LLM 辅助工具',
      llmAssistGenerateHint: '利用下方按钮，引导生成代理根据当前草稿重写题干以符合目标题型，补充必要定义并完善结构化字段。生成完成后请检查结果，系统会尽量保留已有的手动修改。',
      yourMessage: '你的消息',
      send: '发送',
